...
export class OpenAccountPage {
  ...
  async openNewAccount(accountType: AccountType, fromAccountId: string): Promise<string> {
    ...
    await this.page.locator('#fromAccountId option').first().waitFor({ state: 'attached', timeout: 15000 });
    await this.page.selectOption('#fromAccountId', fromAccountId);

    await this.page.getByRole('button', { name: 'Open New Account' }).click();

    await expect(this.page.getByRole('heading', { name: 'Account Opened!' }),'Open-account flow should land on confirmation page').toBeVisible({ timeout: 25000 });
